// fallimmig SDK · sovereign single-file library · MIT · AI-Native Solutions
// Extracted from fallimmig/index.html · 140544 bytes of source logic
...
(function (root) {
  ...
  (function () {
    function go() { if (typeof FallKit !== 'undefined') FallKit.init({ seedName: "fallimmig" }); }
    else go();
  })();
// ════════════════════════════════════════════════════════════════
// Fallimmig v1.0.0 · prime 743 · MIT
// Sovereign immigration case management + UK law engine for OISC-regulated firms
// INFORMATIONAL · NOT REGULATORY SUBMISSION · NOT LEGAL OPINION
// ════════════════════════════════════════════════════════════════
const TOOLNAME='fallimmig';const VERSION='1.0.0';const PRIME=743;const STORE='fallimmig';const CONFIG_V='fallimmig@'+VERSION;
const DB_VERSION=1;
const STORES=['firms','advisers','clients','cases','advice','corpus','weaves','audit','settings'];
...
const TABS=[
 ...
 {id:'immigration case',name:'Immigration case',ico:'§'},
...
const T0_RULES=[
 ...
 {q:'Conveyancing protocol?',a:'OISC Conveyancing Protocol 2019 (residential). Steps: client onboarding + ID, contract pack, searches (local, water/drainage, environmental, chancel), enquiries, mortgage offer, exchange (10% deposit, contract binding), completion (balance + keys), SDLT return within 14 days, Land Registry application within priority period. CQS accreditation for lenders. OISC Warning Notice on dubious investment schemes.'},
 {q:'Wasted costs orders?',a:'SCA 1981 s.51(6); CPR 46.8. Court may order immigration representative personally liable for wasted costs where conduct was improper, unreasonable or negligent. Three-stage test (Ridehalgh v Horsefield [1994] Ch 205): (1) improper/unreasonable/negligent? (2) caused waste? (3) just? High threshold; show cause hearing required. Privilege may need waiving by client to defend.'},
...
const WEAVES=[
 ...
 {id:'W005',name:'The Conflict Wall',archetype:'Sister firm acted for opposing party — instruct fresh',agents:['GUILD','EQUITY'],move:'Information barrier built before instruction — Bolkiah test compliance.',authorities:['Prince Jefri Bolkiah v KPMG [1999] 2 AC 222','OISC Code of Conduct Pt 6','OISC Standards & Regs 2019'],opposition_move:'Former client seeks injunction to restrain.',counter:'Demonstrate effective information barrier: physical, electronic, signed undertakings, no client crossover, fee splits.',why_it_wins:'Bolkiah requires real and substantial risk of disclosure; effective Chinese wall rebuts it.',example_case:'Re Z [2009] EWHC 3621 (Ch) — barrier upheld.'},
 {id:'W006',name:'The CFA Lifeline',archetype:'Client cannot fund litigation — no win, no fee needed',agents:['PROCEDURE','LIBERTY'],move:'Draft compliant CFA: signed before work, base costs + success fee uplift up to 100% of base; cap on damages.',authorities:['Courts & Immigration Services Act 1990 s.58','CFA Order 2013','LASPO 2012'],opposition_move:'Costs judge says CFA unenforceable — non-compliant.',counter:'Hollins v Russell [2003] EWCA Civ 718 — only material breach renders unenforceable; cure trivial defects.',why_it_wins:'CFAs survive technical errors unless integrity of agreement compromised.',example_case:'Hollins v Russell (4 conjoined appeals — most CFAs upheld).'},
...
const CORPUS=[
 {id:'sa1974',title:'Solicitors Act 1974',area:'wills-probate',summary:'Foundational statute regulating solicitors bills (ss.69-70), client account (now OISC Rules), and disciplinary jurisdiction.',keyProvisions:['s.69 form of bill','s.70 client right to assessment','s.74 county court assessment']},
 {id:'lsa2007',title:'Immigration Services Act 2007',area:'other',summary:'Modernised immigration services regulation; created Immigration Services Board, LeO, ABS structures; defined reserved immigration activities.',keyProvisions:['s.12 reserved activities','s.18 entitled persons','Part 5 ABS licensing']},
 ...
 {id:'pace1984',title:'Police and Criminal Evidence Act 1984',area:'crime',summary:'Stop and search powers; arrest; detention timing; access to immigration advice (s.58); confession admissibility (s.76); unfair evidence (s.78).',keyProvisions:['s.24 arrest','s.58 access to advice','s.76 confessions','s.78 unfair evidence']},
...
let state={
 active:'dashboard',immigration caseTab:'overview',activeImmigration caseId:null,
 firm:null,advisers:[],clients:[],cases:[],advice:[],audit:[],
...
async function loadAllStores(){
 ...
 state.cases=(await idbAll('cases'))||[];
 ...
 if(meta){state.activeImmigration caseId=meta.activeImmigration caseId||null;state.active=meta.active||'dashboard';state.chat=meta.chat||[];state.filters=meta.filters||state.filters;state.immigration caseTab=meta.immigration caseTab||'overview'}
}
async function saveMeta(){
 await idbPut('settings',{activeImmigration caseId:state.activeImmigration caseId,active:state.active,chat:state.chat.slice(-100),filters:state.filters,immigration caseTab:state.immigration caseTab},'meta');
}
...
async function audit(action,reasoning,payload){
 ...
 const immigration caseId=(payload&&payload.immigration caseId)||state.activeImmigration caseId||'';
 const clientId=(payload&&payload.clientId)||'';
 const payloadStr=JSON.stringify(payload||{});
 const docHash=await sha256(prevHash+ts+action+adviserId+clientId+immigration caseId+payloadStr);
 const entry={id:'au_'+i+'_'+ts,i,ts,tool:TOOLNAME,adviserId,clientId,immigration caseId,action,reasoning:reasoning||'',configVersion:CONFIG_V,prevHash,docHash,payload:payload||{}};
...
async function saveImmigration case(m,reason,noBc){m.updatedAt=Date.now();await idbPut('cases',m);if(!state.cases.find(x=>x.id===m.id))state.cases.push(m);else state.cases=state.cases.map(x=>x.id===m.id?m:x);await audit('immigration case.updated',reason||'Immigration case updated',{id:m.id,immigration caseId:m.id,clientId:m.clientId,ref:m.ref,title:m.title});if(!noBc)broadcastDeb('immigration case.updated',m)}
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Immigration opinion signed and retained per OISC 13.5',{id:a.id,immigration caseId:a.immigration caseId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
async function handleMesh(m){
 ...
 broadcast('sync.snapshot',{clients:state.clients,advisers:state.advisers,firm:state.firm,cases:state.cases});
 ...
 for(const mt of(p.cases||[])){const local=state.cases.find(x=>x.id===mt.id);if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveImmigration case(mt,'sync from '+m.source,true)}}
 render();
 ...
 }else if(m.type==='immigration case.created'||m.type==='immigration case.updated'){
 const mt=m.payload;if(!mt||!mt.id)return;
 const local=state.cases.find(x=>x.id===mt.id);
 if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveImmigration case(mt,'mesh '+m.type+' from '+m.source,true);render()}
...
function scanConflicts(q){
 ...
 for(const m of state.cases){
 if(opp&&m.partyOpposing&&m.partyOpposing.toLowerCase().includes(opp))hits.push({type:'opposing-party-match',immigration caseId:m.id,party:m.partyOpposing});
 }
 return hits;
}
// ════════════════════════════════════════════════════════════════
...
function newClientRec(){return{
 ...
 cdd:{identityVerifiedMethod:'',addressVerifiedMethod:'',identityVerifiedAt:null,identityVerifiedBy:'',beneficialOwners:[],psc:[],sourceOfFundsForImmigration case:''}},
...
function newImmigration caseRec(clientId){
 const seq=(state.firm&&state.firm.nextImmigration caseRefSeq)||(state.cases.length+1);
 if(state.firm)state.firm.nextImmigration caseRefSeq=seq+1;
 ...
 title:'New immigration case',practiceArea:'civil-litigation',
...
function activeImmigration case(){return state.cases.find(m=>m.id===state.activeImmigration caseId)}
function immigration caseClient(m){return m?state.clients.find(c=>c.id===m.clientId):null}
async function seedDemo(){
 ...
 const mt=newImmigration caseRec(cl.id);
 ...
 mt.demo=true;
 await saveImmigration case(mt,'demo seed',true);
 state.activeImmigration caseId=mt.id;
}
// ════════════════════════════════════════════════════════════════
// VIEW
// ════════════════════════════════════════════════════════════════
function setTab(id){state.active=id;saveMeta();render()}
function setImmigration caseTab(id){state.immigration caseTab=id;saveMeta();render()}
function selectImmigration case(id){state.activeImmigration caseId=id;state.active='immigration case';state.immigration caseTab='overview';saveMeta();render();const sb=$('#sidebar');if(sb)sb.classList.remove('open')}
function renderHeader(){
 ...
 const am=activeImmigration case();
 if(am){$('#mpWho').textContent=am.title;$('#mpMeta').textContent=am.ref+' · '+am.practiceArea}
 else{$('#mpWho').textContent='no immigration case selected';$('#mpMeta').textContent='choose from sidebar'}
 $('#seal').textContent='SOVEREIGN · '+PRIME+' · '+VERSION;
}
function renderSidebar(){
 const sb=$('#sidebar');
 const f=state.filters;
 const cases=state.cases.filter(m=>{
 if(f.q){const s=(m.title+' '+m.ref+' '+m.partyOpposing+' '+(immigration caseClient(m)?clientLabel(immigration caseClient(m)):'')).toLowerCase();if(!s.includes(f.q.toLowerCase()))return false}
 ...
 sb.innerHTML=`
 <h4>Cases · ${state.cases.length}</h4>
 ...
 <button class="btn primary sm" onclick="newImmigration case()">+ new immigration case</button>
 </div>
 <div class="immigration case-list">${cases.length?cases.map(m=>{
 const cl=immigration caseClient(m);
 ...
 return `<div class="immigration case-card ${state.activeImmigration caseId===m.id?'active':''}" onclick="selectImmigration case('${m.id}')">
 ...
 </div>`}).join(''):'<div class="empty-state">no cases yet · click + new immigration case</div>'}</div>`;
}
function render(){
 ...
 else if(a==='cases')v.innerHTML=viewCases();
 else if(a==='immigration case')v.innerHTML=viewImmigration case();
...
function disclaimer(){return `<div class="disclaimer"><strong>Fallimmig</strong> is a tool for OISC-regulated UK solicitors. It assists with immigration case management, CDD, document generation, and OISC Accounts Rules tracking. It is <strong>not regulatory submission or immigration opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <h1>Welcome to Fallimmig</h1>
 <p class="lead">Sovereign immigration case management + UK law research for 1-10 person OISC-regulated firms. First, your firm details.</p>
 ...
 <div class="row"><div class="field"><label>OISC number *</label><input id="f_sra" placeholder="123456"></div><div class="field"><label>Companies House</label><input id="f_ch"></div></div>
...
function onbStep2(){
 ...
 <h1>First adviser</h1>
 <p class="lead">Add yourself — OISC roll number and which compliance roles you hold (COLP/COFA mandatory for the firm).</p>
 ...
 <div class="row"><div class="field"><label>OISC roll number *</label><input id="a_sra"></div><div class="field"><label>Practising cert expiry</label><input type="date" id="a_exp"></div></div>
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>solicitor</option><option>partner</option><option>paraimmigration</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function viewDashboard(){
 const mActive=state.cases.filter(m=>m.status==='active'||m.status==='pending').length;
 const mClosed=state.cases.filter(m=>m.status==='closed').length;
 const critical=state.cases.filter(m=>{
 ...
 const areaBreakdown={};for(const m of state.cases){areaBreakdown[m.practiceArea]=(areaBreakdown[m.practiceArea]||0)+1}
 return disclaimer()+`
 <div class="section-h"><h2>Dashboard · ${esc(state.firm.name)}</h2><div class="sub">OISC ${esc(state.firm.sraNumber||'—')}</div></div>
 <div class="grid">
 <div class="card"><h3>Cases</h3>
 ...
 <div class="kpi"><span class="l">Total</span><span class="v">${state.cases.length}</span></div>
 ...
 ${Object.entries(areaBreakdown).map(([k,v])=>{const pa=PRACTICE_AREAS.find(p=>p.id===k);return `<div class="kpi"><span class="l">${esc(pa?pa.name:k)}</span><span class="v">${v}</span></div>`}).join('')||'<div class="empty-state">no cases</div>'}
 </div>
 ...
 <div class="kpi"><span class="l">Retention</span><span class="v brass">OISC · 6 years</span></div>
 </div>
 </div>
 ...
 const worst=bits[0];
 return `<div class="crit-banner ${worst&&worst.d<=7?'red':''}">⚠ <strong>${esc(m.ref)}</strong> · ${esc(m.title)} · ${bits.map(b=>`${b.k} ${dueLabel(b.d)} (${b.date})`).join(' · ')} <button class="btn sm" style="margin-left:12px" onclick="selectImmigration case('${m.id}')">open →</button></div>`
...
// ════════════════════════════════════════════════════════════════
function viewCases(){
 const mm=state.cases.slice().sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
 return disclaimer()+`
 <div class="section-h"><h2>Cases · ${mm.length}</h2><div class="actions"><button class="btn primary" onclick="newImmigration case()">+ new immigration case</button></div></div>
 ...
 ${mm.length?mm.map(m=>{const cl=immigration caseClient(m);const dL=daysUntil(m.limitationDate);const dS=daysUntil(m.nextStepDue);const pa=PRACTICE_AREAS.find(p=>p.id===m.practiceArea);return `<tr onclick="selectImmigration case('${m.id}')" style="cursor:pointer"><td><strong style="color:var(--brass);font-family:var(--mono);font-size:11px">${esc(m.ref)}</strong></td><td>${esc(m.title)}</td><td>${esc(cl?clientLabel(cl):'—')}</td><td><span class="tag-area">${esc(pa?pa.name:m.practiceArea)}</span></td><td>${esc(m.status)}</td><td style="color:${dL!=null&&dL<=7?'var(--red)':dL!=null&&dL<=28?'var(--amber)':'var(--cream-dim)'}">${m.limitationDate||'—'}${dL!=null?' ('+dueLabel(dL)+')':''}</td><td style="color:${dS!=null&&dS<=7?'var(--red)':'var(--cream-dim)'}">${m.nextStepDue||'—'}${dS!=null?' ('+dueLabel(dS)+')':''}</td><td>${esc(adviserName(m.responsibleSolicitorId))}</td></tr>`}).join(''):'<tr><td colspan="8"><div class="empty-state">no cases · click + new immigration case</div></td></tr>'}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// MATTER · tabs
// ════════════════════════════════════════════════════════════════
function viewImmigration case(){
 const m=activeImmigration case();
 if(!m)return disclaimer()+'<div class="no-active"><h3>No immigration case selected</h3><p>Choose one from the sidebar, or <a onclick="newImmigration case()">create new</a>.</p></div>';
 const cl=immigration caseClient(m);
 ...
 <div class="actions">
 <button class="btn sm ghost" onclick="closeImmigration case('${m.id}')">${m.status==='closed'?'reopen':'close immigration case'}</button>
 <button class="btn sm ox" onclick="deleteImmigration case('${m.id}')">delete</button>
 </div>
 </div>
 <div class="immigration case-tabs">${tabs.map(t=>`<button class="${state.immigration caseTab===t[0]?'active':''}" onclick="setImmigration caseTab('${t[0]}')">${t[1]}</button>`).join('')}</div>
 ${state.immigration caseTab==='overview'?tabOverview(m,cl):''}
 ${state.immigration caseTab==='retainer'?tabRetainer(m):''}
 ${state.immigration caseTab==='conflict'?tabConflict(m,cl):''}
 ${state.immigration caseTab==='timeline'?tabTimeline(m):''}
 ${state.immigration caseTab==='advice'?tabAdvice(m):''}
 ${state.immigration caseTab==='documents'?tabDocs(m):''}
 ${state.immigration caseTab==='fees'?tabFees(m):''}
 `;
}
function tabOverview(m,cl){
 ...
 return `
 <div class="grid g2">
 <div class="card"><h3>Immigration case detail</h3>
 <div class="row"><div class="field"><label>Title</label><input value="${esc(m.title)}" oninput="updImmigration case('title',this.value)"></div><div class="field"><label>Status</label><select onchange="updImmigration case('status',this.value)">${['pending','active','dormant','closed','archived'].map(s=>`<option value="${s}" ${m.status===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Practice area</label><select onchange="updImmigration case('practiceArea',this.value)">${PRACTICE_AREAS.map(p=>`<option value="${p.id}" ${m.practiceArea===p.id?'selected':''}>${p.name}</option>`).join('')}</select></div><div class="field"><label>Risk rating</label><select onchange="updImmigration case('riskRating',this.value)">${['standard','enhanced','high'].map(s=>`<option value="${s}" ${m.riskRating===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Responsible solicitor</label><select onchange="updImmigration case('responsibleSolicitorId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.responsibleSolicitorId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Supervising partner</label><select onchange="updImmigration case('supervisingPartnerId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.supervisingPartnerId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Client</label><select onchange="updImmigration case('clientId',this.value)"><option value="">—</option>${state.clients.map(c=>`<option value="${c.id}" ${m.clientId===c.id?'selected':''}>${esc(clientLabel(c))}</option>`).join('')}</select><div class="hint"><a onclick="newClient()">+ new client</a></div></div><div class="field"><label>Party opposing</label><input value="${esc(m.partyOpposing)}" oninput="updImmigration case('partyOpposing',this.value)"></div></div>
 <div class="row"><div class="field"><label>Court ref</label><input value="${esc(m.courtRef)}" oninput="updImmigration case('courtRef',this.value)"></div></div>
 </div>
...
function tabRetainer(m){
 ...
 <div class="field"><label>Retainer scope</label><textarea oninput="updImmigration case('retainerScope',this.value)">${esc(m.retainerScope)}</textarea></div>
 <div class="field" style="margin-top:8px"><label>Retainer limits / exclusions</label><textarea oninput="updImmigration case('retainerLimits',this.value)">${esc(m.retainerLimits)}</textarea></div>
 <div class="row r3" style="margin-top:8px"><div class="field"><label>Fee arrangement</label><select onchange="updImmigration case('feeArrangement',this.value)">${['hourly','fixed','conditional','damages-based','immigration-aid'].map(s=>`<option value="${s}" ${m.feeArrangement===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Hourly rate £</label><input type="number" value="${m.hourlyRate}" oninput="updImmigration case('hourlyRate',+this.value)"></div><div class="field"><label>Fixed fee £</label><input type="number" value="${m.fixedFee}" oninput="updImmigration case('fixedFee',+this.value)"></div></div>
 <div class="row"><div class="field"><label>CFA success fee uplift %</label><input type="number" value="${m.cfaSuccessFeePct}" oninput="updImmigration case('cfaSuccessFeePct',+this.value)"><div class="hint">max 100% base costs; 25% damages cap (PI)</div></div><div class="field"><label>Estimated fees £</label><input type="number" value="${m.estimatedFees}" oninput="updImmigration case('estimatedFees',+this.value)"></div></div>
...
function tabConflict(m,cl){
 ...
 <div class="row r3"><div class="field"><label>Status</label><select onchange="updImmigration case('conflictStatus',this.value)">${['pending','clear','conflict-identified','conflict-waived'].map(s=>`<option value="${s}" ${m.conflictStatus===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Checked by</label><select onchange="updImmigration case('conflictCheckedBy',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.conflictCheckedBy===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Checked at</label><input type="date" value="${dateIn(m.conflictCheckedAt)}" onchange="updImmigration case('conflictCheckedAt',this.value?new Date(this.value).getTime():null)"></div></div>
 <div class="field" style="margin-top:8px"><label>Notes</label><textarea oninput="updImmigration case('conflictNotes',this.value)">${esc(m.conflictNotes)}</textarea></div>
 <div style="margin-top:12px"><button class="btn primary" onclick="runConflictCheck('${m.id}')">Run conflict scan now</button> <span style="font-size:11px;color:var(--cream-muted);margin-left:10px">scans local cases + emits conflict.check.request on fall-law</span></div>
 ...
 <div class="kpi"><span class="l">Source of funds (immigration case)</span><span class="v">${esc((cl.kyc.cdd&&cl.kyc.cdd.sourceOfFundsForImmigration case)||cl.kyc.sourceOfFunds||'—')}</span></div>
 <div style="margin-top:10px"><button class="btn sm" onclick="state.active='clients';saveMeta();render()">→ open client record</button></div>
 `:'<div class="empty-state">no client linked to this immigration case</div>'}
 </div>`;
}
function tabTimeline(m){
 ...
 <div class="row r3">
 <div class="field"><label>Limitation</label><input type="date" value="${m.limitationDate||''}" onchange="updImmigration case('limitationDate',this.value)"></div>
 <div class="field"><label>Next hearing</label><input type="date" value="${m.nextHearingDate||''}" onchange="updImmigration case('nextHearingDate',this.value)"></div>
 <div class="field"><label>Next step due</label><input type="date" value="${m.nextStepDue||''}" onchange="updImmigration case('nextStepDue',this.value)"></div>
 </div>
...
function tabAdvice(m){
 const advs=state.advice.filter(a=>a.immigration caseId===m.id).sort((a,b)=>(b.ts||0)-(a.ts||0));
 return `<div class="card"><h3>Issue immigration opinion <span class="meta">OISC 13.5 retention · 6yrs</span></h3>
...
function tabDocs(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Document storage lives in <strong>fallimmigpaper</strong> (sister tool on this device). This tab shows attached file refs.</p>
...
function tabFees(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Time recording and billing lives in <strong>fallimmigpractice</strong>. This view shows ledger ref IDs.</p>
...
function viewAdvisers(){
 ...
 <table class="tbl"><thead><tr><th>Name</th><th>Role</th><th>OISC roll</th><th>PC expiry</th><th>COLP</th><th>COFA</th><th>CPD</th><th></th></tr></thead><tbody>
...
function viewFirm(){
 ...
 <div class="row"><div class="field"><label>OISC number</label><input value="${esc(f.sraNumber)}" oninput="state.firm.sraNumber=this.value;saveFirm()"></div><div class="field"><label>Companies House</label><input value="${esc(f.companiesHouseNo)}" oninput="state.firm.companiesHouseNo=this.value;saveFirm()"></div></div>
...
function viewWeaves(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Weaves · ${WEAVES.length}</h2><div class="sub">strategic immigration patterns</div></div>
 ...
 <div style="margin-top:14px">${ws.map(w=>`<div class="weave"><h4>${esc(w.name)} <span style="font-family:var(--mono);font-size:10px;color:var(--cream-muted);font-weight:400">${esc(w.id)}</span></h4><div class="arch">${esc(w.archetype)}</div><div class="seg"><b>Move</b>${esc(w.move)}</div><div class="seg"><b>Authorities</b><div class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</div></div><div class="seg"><b>Opposition move</b>${esc(w.opposition_move)}</div><div class="seg"><b>Counter</b>${esc(w.counter)}</div><div class="seg"><b>Why it wins</b>${esc(w.why_it_wins)}</div><div class="seg"><b>Example</b>${esc(w.example_case)}</div><div class="agents">${w.agents.map(a=>`<span>${esc(a)}</span>`).join('')}</div>${state.activeImmigration caseId?`<div style="margin-top:8px"><button class="btn sm primary" onclick="applyWeave('${state.activeImmigration caseId}','${w.id}')">apply to active immigration case</button></div>`:''}</div>`).join('')}</div>`;
}
function viewCorpus(){
...
function viewQA(){
 ...
 <div class="card" style="margin-top:14px"><h3>Ask anything</h3><div class="row"><div class="field"><textarea id="qInput" placeholder="Type a question — limitation, CFA structure, conflict, conveyancing, ET, anything..."></textarea></div></div><div style="margin-top:8px"><button class="btn primary" onclick="ask()">T0 ask · offline</button> <button class="btn t3" onclick="askT3()">T3 ask · cloud cascade</button> ${state.activeImmigration caseId?`<span style="margin-left:12px;font-size:11px;color:var(--brass)">context: ${esc(activeImmigration case().title)}</span>`:''}</div></div>
...
function viewAudit(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Audit · P3</h2><div class="sub">${state.audit.length} entries · 6yr retention · OISC 13.5</div><div class="actions"><button class="btn" onclick="exportAudit()">export JSON</button></div></div>
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:12px">Hash chain: prevHash + docHash + reasoning. Each entry timestamped, ties to adviser / client / immigration case.</p>
 <table class="tbl"><thead><tr><th>#</th><th>When</th><th>Action</th><th>Adviser</th><th>Immigration case</th><th>Reason</th><th>Hash</th></tr></thead><tbody>
 ${recent.map(a=>{const mt=state.cases.find(x=>x.id===a.immigration caseId);return `<tr><td>${a.i}</td><td>${dateStr(a.ts)}</td><td><strong>${esc(a.action)}</strong></td><td>${esc(adviserName(a.adviserId))}</td><td>${esc(mt?mt.ref:'—')}</td><td>${esc(a.reasoning)}</td><td style="font-family:var(--mono);font-size:9px">${esc((a.docHash||'').slice(0,16))}…</td></tr>`}).join('')}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// ACTIONS
// ════════════════════════════════════════════════════════════════
function updImmigration case(field,val){const m=activeImmigration case();if(!m)return;m[field]=val;saveImmigration case(m,'field '+field+' updated')}
async function newImmigration case(){
 const m=newImmigration caseRec();state.activeImmigration caseId=m.id;
 await saveImmigration case(m,'new immigration case');await audit('immigration case.created','New immigration case created',{id:m.id,immigration caseId:m.id,ref:m.ref});broadcast('immigration case.created',m);
 state.active='immigration case';state.immigration caseTab='overview';saveMeta();render();
}
async function closeImmigration case(id){const m=state.cases.find(x=>x.id===id);if(!m)return;m.status=m.status==='closed'?'active':'closed';m.closedAt=m.status==='closed'?Date.now():null;await saveImmigration case(m,'immigration case '+m.status);if(m.status==='closed')broadcast('immigration case.closed',m);else broadcast('immigration case.reopened',m);render()}
async function deleteImmigration case(id){if(!confirm('Delete this immigration case? (audit log preserved)'))return;await idbDel('cases',id);state.cases=state.cases.filter(m=>m.id!==id);if(state.activeImmigration caseId===id)state.activeImmigration caseId=null;await audit('immigration case.deleted','Immigration case deleted',{id});render()}
async function sendClientCare(id){const m=state.cases.find(x=>x.id===id);if(!m)return;m.clientCareSentAt=Date.now();await saveImmigration case(m,'client care letter marked sent');toast('client care marked sent');render()}
async function addOutcome(id){const m=state.cases.find(x=>x.id===id);if(!m)return;const t=$('#tlText').value.trim();if(!t)return;m.outcomes=m.outcomes||[];m.outcomes.push({ts:Date.now(),adviserId:state.settings.currentAdviserId,text:t});await saveImmigration case(m,'event added');render()}
async function addFileRef(id){const m=state.cases.find(x=>x.id===id);if(!m)return;const n=$('#frefIn').value.trim();if(!n)return;m.fileRefs=m.fileRefs||[];m.fileRefs.push({name:n,ts:Date.now()});await saveImmigration case(m,'file ref attached');render()}
async function rmFileRef(id,i){const m=state.cases.find(x=>x.id===id);if(!m)return;m.fileRefs.splice(i,1);await saveImmigration case(m,'file ref removed');render()}
async function issueAdvice(immigration caseId){
 const m=state.cases.find(x=>x.id===immigration caseId);if(!m)return;
 ...
 const sig=await sha256(immigration caseId+adviserId+ts+txt);
 const a={id:'adv_'+uid(),immigration caseId,clientId:m.clientId,adviserId,ts,text:txt,signature:sig};
 await saveAdvice(a);broadcast('advice.issued',a);toast('opinion signed and retained');$('#advText').value='';render();
}
async function applyWeave(immigration caseId,wid){const m=state.cases.find(x=>x.id===immigration caseId);if(!m)return;m.appliedWeaves=m.appliedWeaves||[];if(!m.appliedWeaves.includes(wid))m.appliedWeaves.push(wid);await saveImmigration case(m,'weave '+wid+' applied');toast('weave applied');render()}
async function unapplyWeave(immigration caseId,wid){const m=state.cases.find(x=>x.id===immigration caseId);if(!m)return;m.appliedWeaves=(m.appliedWeaves||[]).filter(x=>x!==wid);await saveImmigration case(m,'weave '+wid+' removed');render()}
function showWeave(wid){const w=WEAVES.find(x=>x.id===wid);if(!w)return;modal(`<h3>${esc(w.name)}</h3><p style="font-style:italic;color:var(--cream-dim);margin-bottom:10px">${esc(w.archetype)}</p><div class="seg"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Move</b><br>${esc(w.move)}</div><div class="seg" style="margin-top:8px"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Authorities</b><br><span class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</span></div><div class="seg" style="margin-top:8px"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Counter</b><br>${esc(w.counter)}</div><div style="margin-top:14px">${state.activeImmigration caseId?`<button class="btn primary" onclick="applyWeave('${state.activeImmigration caseId}','${w.id}');closeModal()">apply to active immigration case</button> `:''}<button class="btn ghost" onclick="closeModal()">close</button></div>`)}
async function runConflictCheck(immigration caseId){
 const m=state.cases.find(x=>x.id===immigration caseId);if(!m)return;
 const cl=immigration caseClient(m);
 ...
 await saveImmigration case(m,'conflict scan run');
...
function answerT0(q){
 ...
 ans+='\n\n— Fallimmig T0 · informational, verify against primary sources.';
 return ans;
}
async function askT3(){
 const txt=$('#qInput').value.trim();if(!txt){toast('type a question first');return}
 const m=activeImmigration case();const cl=m?immigration caseClient(m):null;
 const ctx=m?`\nActive immigration case: ${m.ref} — ${m.title} (${m.practiceArea}). Client: ${cl?clientLabel(cl):'—'}.`:'';
 state.chat.push({role:'user',text:txt,ts:Date.now()});saveMeta();render();
 const sys='You are a UK immigration research assistant for OISC-regulated solicitors. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated immigration advice — research only. Always say "verify against primary sources".';
 ...
 state.chat.push({role:'bot',text:answer+'\n\n— Fallimmig T3 · BYOK · verify against primary sources.',ts:Date.now()});saveMeta();render();$('#qInput').value='';
}
...
function exportAll(){downloadJSON('fallimmig-export-'+new Date().toISOString().slice(0,10)+'.json',{firm:state.firm,advisers:state.advisers,clients:state.clients,cases:state.cases,advice:state.advice,audit:state.audit,exportedAt:Date.now(),tool:TOOLNAME,version:VERSION});toast('exported')}
function exportAudit(){downloadJSON('fallimmig-audit-'+new Date().toISOString().slice(0,10)+'.json',state.audit);toast('audit exported')}
async function purgeDemo(){
 const dM=state.cases.filter(m=>m.demo);const dC=state.clients.filter(c=>c._demo);
 for(const m of dM){await idbDel('cases',m.id)}for(const c of dC){await idbDel('clients',c.id)}
 state.cases=state.cases.filter(m=>!m.demo);state.clients=state.clients.filter(c=>!c._demo);
 if(state.activeImmigration caseId&&!state.cases.find(m=>m.id===state.activeImmigration caseId))state.activeImmigration caseId=null;
 await audit('demo.purged','Demo data removed',{cases:dM.length,clients:dC.length});toast('demo purged');render();
}
...
(async function boot(){
 ...
 render();
 await audit('app.boot','Fallimmig booted',{version:VERSION,prime:PRIME});
})();
